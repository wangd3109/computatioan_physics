...
!subroutine,处理得到磁矩？h1（哈密顿量）？对初始化并翻转后的lattice计算此时的性质（磁性，哈密顿量）
      call hamil(grid,jex,b,lattice,mag,h1,su,sd,sl,sr)
      summationh=h1
!      mag=abs(mag)
      summationm=mag
      cont=1
...
!随机翻转一个格点上的磁矩，并计算磁化强度，哈密顿量，循环指定步数
      ...
      call hamil(grid,jex,b,lattice,mag,h2,su,sd,sl,sr)
!      mag=abs(mag)
...
!计算第二项，磁场对能量的贡献，对spin求和
        ...
        h=-jex*summation1-b*mag !jex1 only
!        h=-(jex*summation1+jex2*summation2)-b*mag		!jex1 and jex2
